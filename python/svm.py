    ...
    if scipy and isinstance(xi, tuple) and len(xi) == 2\
      and isinstance(xi[0], scipy.ndarray) and isinstance(xi[1], scipy.ndarray): # for a sparse vector
        ...
        if not isKernel:
            ...
            index_range = scipy.arange(
                0, len(xi))  # index starts from 0 for precomputed kernel
        if feature_max:
            index_range = index_range[scipy.where(index_range <= feature_max)]
...
def csr_to_problem_nojit(l, x_val, x_ind, x_rowptr, prob_val, prob_ind,
                         prob_rowptr, indx_start):
    for i in range(l):
...
def csr_to_problem(x, prob, isKernel):
    ...
    if jit_enabled:
        csr_to_problem_jit(x.shape[0], x.data, x.indices, x.indptr, prob_val,
                           prob_ind, prob.rowptr, indx_start)
    else:
        csr_to_problem_nojit(x.shape[0], x.data, x.indices, x.indptr, prob_val,
                             prob_ind, prob.rowptr, indx_start)
...
class svm_parameter(Structure):
    _names = [
        "svm_type", "kernel_type", "degree", "gamma", "coef0", "cache_size",
        "eps", "C", "nr_weight", "weight_label", "weight", "nu", "p",
        "shrinking", "probability"
    ]
    _types = [
        c_int, c_int, c_int, c_double, c_double, c_double, c_double, c_double,
        c_int,
        POINTER(c_int),
...
class svm_model(Structure):
    _names = [
        'param', 'nr_class', 'l', 'SV', 'sv_coef', 'rho', 'probA', 'probB',
        'sv_indices', 'label', 'nSV', 'free_sv'
    ]
    _types = [
        svm_parameter, c_int, c_int,
    ...
    def get_sv_coef(self):
        return [
            tuple(self.sv_coef[j][i] for j in range(self.nr_class - 1))
...
fillprototype(
    libsvm.svm_train, POINTER(svm_model),
    [POINTER(svm_problem), POINTER(svm_parameter)])
fillprototype(
    libsvm.svm_cross_validation, None,
    [POINTER(svm_problem),
     POINTER(svm_parameter), c_int,
     POINTER(c_double)])
